  ...
  if (cart.length === 0) return (
    <div className="bg-stone-950 min-h-screen text-white flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-stone-400 mb-8">Discover our beautiful collection of handmade rugs</p>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <nav className="flex items-center gap-2 text-xs text-stone-500 mb-2">
              ...
              <span>/</span>
              <span className="text-stone-300">Cart</span>
            </nav>
            <h1 className="text-4xl font-black text-white">Shopping Cart</h1>
            <p className="text-stone-400 mt-1">{cart.reduce((s, i) => s + i.quantity, 0)} items</p>
          </div>
          <button onClick={clearCart} className="text-sm text-stone-500 hover:text-red-400 transition-colors">Clear cart</button>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-xs bg-stone-800 text-stone-400 px-2 py-0.5 rounded-lg">Size: {item.selectedSize}</span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="text-right">
                      ...
                      {item.quantity > 1 && <p className="text-xs text-stone-500">${item.price.toLocaleString()} each</p>}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-stone-900 border border-stone-800 rounded-2xl p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-5">
                ...
                {promoApplied && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-emerald-400 font-semibold">−${discount.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between text-sm">
                ...
                {shipping > 0 && <p className="text-xs text-stone-500">Add ${(500 - cartTotal).toLocaleString()} more for free shipping</p>}
              </div>
              ...
              <div className="mb-5">
                ...
                {promoApplied && <p className="text-xs text-emerald-400 mt-2">✓ Code CARPET20 applied — 20% off!</p>}
                {!promoApplied && <p className="text-xs text-stone-600 mt-2">Try: CARPET20</p>}
              </div>
